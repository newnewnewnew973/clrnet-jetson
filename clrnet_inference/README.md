...
- `clrnet_inference/` 안의 proxy package로 필요한 import만 가로챕니다.
...
├── clrnet_common/                  # 공통 runtime, CULane, metric, latency, CUDA NMS
├── clrnet_inference/               # PyTorch inference package
├── clrnet_tensorrt/                # TensorRT conversion/runtime package
├── data/CULane/                     # CULane dataset, git에는 포함하지 않음
└── weights/culane_dla34.pth         # DLA34 checkpoint, git에는 포함하지 않음
```

`clrnet_inference/` 내부 구조:
...
공통 runtime, CULane dataset helper, metric, latency helper, CUDA NMS extension은
workspace root의 `clrnet_common/`에 둡니다. PyTorch와 TensorRT가 같은 전처리,
decode/NMS, metric 기준을 공유하도록 만든 구조입니다.
...
`clrnet_inference/data/CULane`은 `data/CULane`을 가리키는 symlink로 둡니다.
...
- `clrnet.ops.nms` 경로는 `clrnet_inference/` proxy가 받고, 실제 NMS 구현은 `clrnet_common/`을 사용합니다.
- `clrnet.models`, `clrnet.datasets`, `clrnet.utils`는 공식 `./clrnet/clrnet`에서 import됩니다.
- `mmcv`, `torchvision`은 `clrnet_inference/`의 최소 shim을 사용합니다.
...
```text
clrnet_inference/outputs/single_image/*_clrnet_<model>.jpg
clrnet_inference/outputs/single_image/*.lines.txt
```
...
python clrnet_inference/scripts/evaluate_culane.py \
  ...
  --device cuda \
  --batch-size 4 \
  --output-dir clrnet_inference/outputs/eval/dla34_full
```
...
```text
clrnet_inference/outputs/eval/dla34_<YYYYmmdd_HHMMSS>/**/*.lines.txt
```
...
Metric 계산 로직은 `clrnet_common`에 있고, PyTorch 경로에서는 아래 script로 실행합니다.
...
python clrnet_inference/scripts/measure_pytorch_latency.py \
  ...
  --device cuda \
  --data-root clrnet_inference/data/CULane \
  --output-dir clrnet_inference/outputs/latency/pytorch_dla34_1000 \
  --limit 1000 \
  --warmup 100 \
  --block-warmup 10
```
...
```text
clrnet_inference/outputs/latency/pytorch_dla34/pytorch_latency_dla34.json
clrnet_inference/outputs/latency/pytorch_dla34/PYTORCH_LATENCY_BASELINE.md
```

최근 1000개 sample 기준 측정 결과:

```text
fps_continuous_e2e                 12.72
fps_continuous_forward             32.41
fps_continuous_forward_postprocess 27.05
fps_cuda_event_forward             31.39
latency_cuda_event_forward_mean_ms 31.86
latency_continuous_e2e_ms          78.59
```
...
이 README를 갱신할 때 다음 검증을 완료했습니다.

```bash
python clrnet_inference/scripts/inference_culane.py \
  --model dla34 \
  --image data/CULane/driver_100_30frame/05251517_0433.MP4/00000.jpg \
  --output-dir clrnet_inference/outputs/single_image/full_run \
  --device cuda
...
python clrnet_inference/scripts/measure_culane_metric.py \
  --model dla34 \
  --pred-dir clrnet_inference/outputs/eval/dla34_full \
  --data-root clrnet_inference/data/CULane \
  --output-json clrnet_inference/outputs/eval/dla34_full_metric_0_5.json \
  --workers 4 \
  --progress-interval 500
```
...
```text
single_image_lanes=3
predictions_written=34680
prediction_files=34680
metric_json=clrnet_inference/outputs/eval/dla34_full_metric_0_5.json
latency_json=clrnet_inference/outputs/latency/pytorch_dla34_1000/pytorch_latency_dla34.json
```
...
- `clrnet/`: local proxy package입니다. `clrnet.ops.nms` import를 받아
  `clrnet_common`의 CUDA NMS로 넘기고, 나머지 `clrnet.models`, `clrnet.datasets`,
  `clrnet.utils`는 공식 CLRNet으로 넘깁니다.
...
- `data/`
- `weights/`
- `clrnet_inference/data/`
- `clrnet_inference/outputs/`의 실행 결과
